...
// Removed useParams as it will be passed as a prop
...
export default function BlogPostClientPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    if (!isLoading && blogPosts.length > 0) {
      const slug = params.slug as string; // Use prop
      const foundPost = blogPosts.find((p) => p.slug === slug || p.id.toString() === slug);
      setPost(foundPost);
    }
  }, [params.slug, blogPosts, isLoading]); // Use prop in dependency array
  ...
  useEffect(() => {
    ...
    const updateViewCount = () => {
      setBlogPosts((prevPosts: any[]) => prevPosts.map((p) => (p.id === post.id ? { ...p, views: (p.views || 0) + 1 } : p)));
    };
